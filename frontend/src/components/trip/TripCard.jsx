...
function TripCard({ trip }) {
  return (
    <motion.article
      whileHover={{ y: -4 }}
      className="overflow-hidden rounded-lg border border-slate-200 bg-white travel-card-shadow"
    >
      <div className="relative h-44">
        <img src={trip.image} alt={trip.title} className="h-full w-full object-cover" />
        <span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-sky-700">
          {trip.status}
        </span>
      </div>
      <div className="p-4">
        ...
        <Link
          ...
          className="mt-4 inline-flex text-sm font-bold text-sky-700 hover:text-orange-500"
        >
          Open itinerary
        </Link>
      </div>
